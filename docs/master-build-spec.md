...
Automated gate: npm run m1:test (35 groups), npm run social-command:test, and Freedom Audit's eight auth tests plus both production builds. Fixtures create isolated MongoDB replica sets with synthetic users and mocked provider calls. Coverage includes read-only loop status, account allowlisting, founder context/task persistence with execution disabled, cross-user denial and execution separation. The old remote account/role-mutating technical harness invokes these fixtures. PR CI runs these gates. CI's Render step remains workflow_dispatch-only.
